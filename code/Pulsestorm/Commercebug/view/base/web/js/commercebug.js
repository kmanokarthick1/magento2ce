jQueryCb(function(){
    ...
    var insertUiComponentTreeIntoContainer = function(componentName, container)
    {
        ...
        if(!jQueryCb(container).hasClass('cbjstree'))
        {
            jQueryCb(container)
                .on('click', '.cbjstree-anchor', function (e) {
                    jQueryCb(container).cbjstree(true).toggle_node(e.target);
                })
                .cbjstree(
                    { 'core' : {
                        'data' : nodes,
            ...
            jQueryCb(container).bind('ready.cbjstree', function(e){
                jQuery('.cbjstree-container-ul').delegate('a', 'click', function(e){
    ...
    var templateGraphForm = function(data)
    {
        ...
        warnings +
        '<div id="commercebug_layout_buttons">' +
            '<input name="token_as_commercebug" type="hidden" value="<$token$>">' +
            '<button class="commercebug-button" type="submit">Render Graph</button>' +
            '<button class="commercebug-button" onclick="jQueryCb(\'#pulsestorm_commercebug_graph_source_container\').toggle();return false;">Show Graph Source</button>' +
            '<button class="commercebug-button pulsestorm_textbox_but" id="pulsestorm_button_view_handle_layout">View Package/Handle Layout</button>'  +
            '<button class="commercebug-button pulsestorm_textbox_but" id="pulsestorm_button_view_xmlfile">View Loaded XML Files</button>'             +
            '<button class="commercebug-button pulsestorm_textbox_but" id="pulsestorm_button_view_request_layout">View Page/Request Layout</button>'   +
            '<button class="commercebug-button pulsestorm_textbox_but" id="pulsestorm_button_view_structure_schedule">View Structure Schedule</button>'+
        '</div>' +
    ...
    var setupClassLookupTab = function(id)
    {
        ...
        var string = '<h2>Class Lookup</h2><p>Enter a PHP class name and get back object system information.</p><form action="/commercebug/lookup" method="post" target="'+target+'"><input type="text" name="lookup" value="" /><button class="commercebug-button">Submit</button></form>';
        string += '<br>';
        string += '<h2>Knockout.js Lookup</h2>' +
            ...
            '<button class="commercebug-button">Submit</button>' +
