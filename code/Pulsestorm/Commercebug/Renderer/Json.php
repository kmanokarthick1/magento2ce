...
* All rights reserved
*/
namespace Pulsestorm\Commercebug\Renderer;
...
class Json
{
    ...
    */    
    public function clean($data)
    {
        ...
        foreach($data as $key=>$value)
        {
            if($key === 'invoked_observers')
            {
                $data[$key] = array_filter($value, function($val){                    
                    ...
                    return strpos($val['instance'], 'Pulsestorm\\Commercebug') === false;
                });
            }
        ...
        {
            $data['layouts'] = [
                'graph'=> \Pulsestorm\Commercebug\Plugins\MagentoFrameworkViewLayout::renderGraph(),
